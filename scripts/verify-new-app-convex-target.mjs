...
const envFile = envFileIndex >= 0 ? process.argv[envFileIndex + 1] : ".env.local";
if (!envFile || !existsSync(envFile)) {
  throw new Error(`New-app Convex env file not found: ${envFile ?? "<missing>"}`);
}

loadEnvFile(envFile);
const target = JSON.parse(
  readFileSync(new URL("../config/new-app-convex-target.json", import.meta.url), "utf8")
);
...
if (deploymentKind === "local") {
  ...
  if (!normalized.includes(expectedTeam) || !normalized.includes(expectedProject)) {
    throw new Error(
...
if (process.env.SPORTLINK_CLIENT_ID?.trim()) {
  throw new Error("SPORTLINK_CLIENT_ID must not be present in the new-app development env");
}
