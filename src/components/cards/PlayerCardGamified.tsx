...
/** Club black + gold rarity frames (DIA wapen). */
const RARITY_FRAME: Record<string, string> = {
  common: "border-dia-yellow/40 shadow-black/40",
  rare: "border-dia-yellow shadow-[0_0_16px_rgba(232,192,26,0.35)]",
  epic: "border-dia-yellow shadow-[0_0_28px_rgba(232,192,26,0.55)]",
};
...
export function PlayerCardGamified({
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] bg-gradient-to-b from-neutral-900 to-dia-black flex items-center justify-center">
        {photoUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={photoUrl} alt="" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
          <span className="text-5xl font-bold text-dia-yellow/25">
            {number != null ? number : "?"}
          </span>
        )}
      ...
      <div className="p-3 space-y-1 border-t border-dia-yellow/20">
        <h3 className="font-bold text-sm truncate text-white">
          {showFullIdentity ? displayName : displayName}
        </h3>
        {cardProfile ? (
