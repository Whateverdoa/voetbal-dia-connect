...
import type { PitchOrientation } from "@/lib/pitchOrientation";
...
interface FieldLinesProps {
  ...
  /** Show only the own half (bottom). Ignored when orientation is landscape. */
  crop?: "ownHalf";
}
...
export function FieldLines({
  ...
  const ownHalf = crop === "ownHalf" && !landscape;
  ...
  return (
    ...
    >
      {/* All markings are drawn portrait; landscape rotates the whole group so
          the geometry below stays the single source of truth. */}
      <g transform={landscape ? `translate(${h} 0) rotate(90)` : undefined}>
