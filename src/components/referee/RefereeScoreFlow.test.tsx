...
describe("RefereeScoreFlow", () => {
  ...
  it("disables score changes outside a running match", () => {
    render(
      <RefereeScoreFlow
        matchId={"m1" as Id<"matches">} homeName="DIA" awayName="VOAB"
        homeScore={0} awayScore={0} isLoading={false} scoreError={null}
        canRecord={false} cardControls={null} withLoading={vi.fn()} onScoreError={vi.fn()}
      />,
    );
    expect(screen.getByRole("button", { name: "DIA score +1" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "VOAB score +1" })).toBeDisabled();
    expect(screen.queryByRole("button", { name: "Eigen doelpunt" })).not.toBeInTheDocument();
  });
