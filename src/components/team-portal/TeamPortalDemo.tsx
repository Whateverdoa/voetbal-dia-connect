...
import type { DemoActor } from "@/lib/team-portal/types";
...
type Tab = "overview" | "development" | "team" | "voting" | "coach" | "observations";
const phaseLabels = { preparing: "Acties klaarzetten", voting: "Stemmen is open", closed: "Uitslag bekend" };

export function TeamPortalDemo() {
  const { state, actor, setActor, now, ready, run, reset, storageWarning, notice, dismissNotice } = useTeamPortalDemo();
  ...
  const tabs: { id: Tab; label: string; icon: typeof Users }[] = actor.role === "coach"
    ? [{ id: "coach", label: "Ontwikkeling", icon: ClipboardList }, { id: "observations", label: "Observaties", icon: Eye }, { id: "voting", label: "Wedstrijdwaardering", icon: Trophy }, { id: "team", label: "Het team", icon: Users }]
    : actor.role === "scout"
  ...
  const changeRole = (role: DemoActor["role"]) => {
    ...
    setTab(role === "coach" ? "coach" : role === "scout" ? "observations" : "overview");
    dismissNotice();
  ...
  const headings: Record<Tab, { eyebrow: string; title: string; text: string }> = {
    ...
    team: { eyebrow: "DIA JO13 · seizoen 2026–2027", title: "Twaalf spelers. Eén team.", text: "Ieder een eigen kracht. Samen maken we er iets moois van." },
    ...
    <div className="min-h-screen bg-[#f6f7f2] pb-24 text-stone-900 sm:pb-8">
      ...
      <div className="border-b border-[#e7dfce] bg-[#f0ebdf]">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-2.5 sm:px-6">
          <p className="flex items-center gap-2 text-xs font-medium text-[#605742]"><FlaskConical size={15} /><span><strong>DIA Team · demo</strong><span className="hidden sm:inline"> · fictieve spelers, lokaal opgeslagen</span></span></p>
      ...
      <main id="teamportal-content" className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-9">
        ...
        {(activeTab === "voting" || activeTab === "coach") ? <section className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-stone-200 bg-white p-4"><label className="flex flex-wrap items-center gap-3 text-sm font-semibold"><span className="text-stone-500">Wedstrijd</span><select aria-label="Wedstrijd" value={match.id} onChange={(event) => setMatchId(event.target.value)} className="min-h-11 max-w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm font-bold">{state.matches.map((item) => <option key={item.id} value={item.id}>DIA – {item.opponent} · {item.score}</option>)}</select></label><span className={`rounded-full px-3 py-2 text-xs font-bold ${getMatchPhase(match, now) === "voting" ? "bg-green-100 text-green-800" : "bg-stone-100 text-stone-600"}`}>{phaseLabels[getMatchPhase(match, now)]}</span></section> : null}
