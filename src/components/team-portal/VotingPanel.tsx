      ...
      <form onSubmit={savePlayer} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        {players.length === 0 ? <p className="mt-5 text-sm text-slate-500">Er zijn geen andere spelers om op te stemmen.</p> : (
          <>
            ...
            <select id={`${id}-ballot-player`} className={inputClass} value={playerId} onChange={(event) => setPlayerId(event.target.value)} required><option value="">Wie viel jou positief op?</option>{players.map((player) => <option key={player.id} value={player.id}>{player.name}{player.number != null ? ` · #${player.number}` : ""}</option>)}</select>
